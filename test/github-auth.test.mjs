...
function setup(fetchImpl) {
  ...
  const auth = createGithubAuth({ origin: "http://127.0.0.1:4318", callbackUrl: "http://127.0.0.1:4318/claim/auth/github/callback", clientId: "test-client", clientSecret: "test-secret", store, fetchImpl, now: () => 1789990000000 });
...
test("starts GitHub OAuth with state, PKCE, and HttpOnly cookies", async () => {
  ...
  const response = await auth.handle(new Request("http://127.0.0.1:4318/auth/github"), "/claim/auth/github");
  ...
  assert.equal(authorize.searchParams.get("redirect_uri"), "http://127.0.0.1:4318/claim/auth/github/callback");
...
test("callback verifies state, exchanges PKCE code, fetches GitHub identity, and issues a session", async () => {
  ...
  const start = await auth.handle(new Request("http://127.0.0.1:4318/auth/github"), "/claim/auth/github");
  const cookies = responseCookies(start);
  const state = new URL(start.headers.get("location")).searchParams.get("state");
  const callback = await auth.handle(new Request(`http://127.0.0.1:4318/claim/auth/github/callback?code=oauth-code&state=${state}`, { headers: { cookie: cookies } }), "/claim/auth/github/callback");
  assert.equal(callback.status, 303);
  assert.equal(new URL(callback.headers.get("location")).pathname, "/claim");
...
test("rejects state mismatch without contacting GitHub", async () => {
  ...
  const response = await auth.handle(new Request("http://127.0.0.1:4318/claim/auth/github/callback?code=x&state=attacker"), "/claim/auth/github/callback");
...
test("logout checks origin and deletes the hashed session", async () => {
  ...
  const rejected = await auth.handle(new Request("http://127.0.0.1:4318/claim/auth/logout", { method: "POST", headers: { origin: "https://attacker.test", cookie: "yukon_reward_session=session" } }), "/claim/auth/logout");
  assert.equal(rejected.status, 403);
  const response = await auth.handle(new Request("http://127.0.0.1:4318/claim/auth/logout", { method: "POST", headers: { origin: "http://127.0.0.1:4318", cookie: "yukon_reward_session=session" } }), "/claim/auth/logout");
